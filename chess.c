...
void pathfinder(Matrix *matrix, Point *point)
{
    // find all the possible moves for the queen until it hits a piece or the edge of the board
    // store them in array of algebraic notations
    // if piece is opponent's, store the alg notation + "x" + value of piece ('P', 'R', etc.)
    ...
    for (int i = point->y - 1; i >= 0; i--)
    {
        ...
        else if (matrix->points[point->x + i * 8].value >= 'A' && matrix->points[point->x + i * 8].value <= 'Z')
        {
            moves[move_count][0] = matrix->points[point->x + i * 8].algebraic[0];
            moves[move_count][1] = matrix->points[point->x + i * 8].algebraic[1];
    ...
    for (int i = point->x - 1; i >= 0; i--)
    {
        ...
        // if the point is an opponent's piece, add it to the list of moves
        else if (matrix->points[i + point->y * 8].value >= 'A' && matrix->points[i + point->y * 8].value <= 'Z')
        {
            moves[move_count][0] = matrix->points[i + point->y * 8].algebraic[0];
            moves[move_count][1] = matrix->points[i + point->y * 8].algebraic[1];
            moves[move_count][2] = 'x';
    ...
    for (int i = 1; point->x - i >= 0 && point->y - i >= 0; i++)
    {
        ...
        {
            moves[move_count][0] = matrix->points[point->x - i + (point->y - i) * 8].algebraic[0];
            moves[move_count][1] = matrix->points[point->x - i + (point->y - i) * 8].algebraic[1];
            moves[move_count][2] = '\0';
            move_count++;
        }
        // if the point is an opponent's piece, add it to the list of moves
        else if (matrix->points[point->x - i + (point->y - i) * 8].value >= 'A' && matrix->points[point->x - i + (point->y - i) * 8].value <= 'Z')
        {
            moves[move_count][0] = matrix->points[point->x - i + (point->y - i) * 8].algebraic[0];
            moves[move_count][1] = matrix->points[point->x - i + (point->y - i) * 8].algebraic[1];
            moves[move_count][2] = 'x';
            moves[move_count][3] = matrix->points[point->x - i + (point->y - i) * 8].value;
            moves[move_count][4] = '\0';
            move_count++;
            break;
        }
